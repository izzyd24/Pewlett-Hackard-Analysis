...
ce.last_name,
d.dept_name
--INTO retirement_info_salesdept
FROM retirement_info as ri
LEFT JOIN dept_emp as de
ON ri.emp_no = de.emp_no
-- this is a table of current employees, add the filter with where
...
-- Check the table after refresh
SELECT * FROM retirement_info_sales_dept;

-- same manager now wants list for both sales and development dept. for retirement info
